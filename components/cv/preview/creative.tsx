...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-3 text-[11px] font-bold tracking-[0.12em] text-violet-700 uppercase">
      {children}
    </h2>
  );
}
...
function TimelineItem({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative">
      <span className="absolute -left-[29px] top-1 h-2.5 w-2.5 rounded-full border-2 border-violet-600 bg-white" />
      {children}
    </div>
  );
}
...
export function CreativePreview({ data }: { data: CvData }) {
  ...
  return (
    <div className="mx-auto w-full bg-white text-neutral-900 shadow-sm sm:w-[8.5in] sm:max-w-full">
      <header className="border-b-2 border-violet-200 px-8 pt-10 pb-6 text-center sm:px-[0.75in]">
      ...
      <div className="p-8 sm:p-[0.75in] sm:pt-8">
        ...
        {experience.length > 0 && (
          <section className="mb-6">
            ...
            <div className="space-y-4 border-l-2 border-violet-200 pl-6">
              {experience.map((item) => (
